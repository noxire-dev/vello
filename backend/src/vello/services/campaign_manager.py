...
class CampaignManager:
        ...
        try:
            ...
            for email in emails:
                ...
                recipient = Recipient(
                    ...
                    vars_json=str(vars_json.get(email)) if vars_json and vars_json.get(email) else None,
                    suppressed=False,
    ...
    def process_pending_deliveries(self, campaign_id: Optional[int] = None) -> int:
        ...
        try:
            ...
            query = session.query(Delivery).join(Recipient).join(CampaignStep).filter(
            ...
            for delivery in pending_deliveries:
                ...
                if step.position == 0:
                    ...
                    prev_step = session.query(CampaignStep).filter_by(
                        campaign_id=campaign.id,
                        position=step.position - 1
                    ).first()

                    if prev_step:
                        prev_delivery = session.query(Delivery).filter_by(
                            recipient_id=recipient.id,
                            step_id=prev_step.id
                        ).first()

                        if not prev_delivery or prev_delivery.status != DeliveryStatus.SENT:
                            continue

                        if prev_delivery.sent_at:
                            time_since_prev = datetime.utcnow() - prev_delivery.sent_at
                            if time_since_prev.total_seconds() < step.delay_minutes * 60:
                                continue
    ...
    def handle_response(
        ...
        try:
            # Find recipient
            recipient = session.query(Recipient).filter_by(email=recipient_email).first()
            if not recipient:
            ...
            if config.AUTO_UNSUBSCRIBE_ON_REQUEST and status == ResponseStatus.UNSUBSCRIBED:
                recipient.suppressed = True
                # Stop all pending deliveries for this recipient
                pending_deliveries = session.query(Delivery).join(CampaignStep).filter(
    ...
    def get_campaign_stats(self, campaign_id: int) -> Dict[str, Any]:
        ...
        deliveries = session.query(Delivery).join(Recipient).filter(
        ...
        # Count responses
        from vello.core.models import Response
        responses = session.query(Response).join(Recipient).filter(
            Recipient.campaign_id == campaign_id
        ).all()
        ...
        unsubscribed = sum(1 for r in responses if r.status == ResponseStatus.UNSUBSCRIBED)
    ...
    def initialize_campaign_deliveries(self, campaign_id: int) -> int:
        ...
        try:
            ...
            # Get first step
            first_step = session.query(CampaignStep).filter_by(
                campaign_id=campaign_id,
                position=0
            ).first()
